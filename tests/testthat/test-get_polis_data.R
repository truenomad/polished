...
test_that("get_polis_data aborts when the API key is empty", {
  withr::local_envvar(POLIS_API_KEY = "")
  expect_error(
    get_polis_data(tables = "im", polis_api_key = ""),
    "POLIS API key is empty"
  )
})

test_that("get_polis_data aborts on unknown table names", {
  expect_error(
    get_polis_data(
      tables = "not_a_table",
      polis_api_key = "dummy",
...
test_that("get_polis_data aborts on `return = \"df\"` with multi-table", {
  expect_error(
    get_polis_data(
      tables = c("im", "lqas"),
      return = "df",
...
test_that("polis_tables_mapping has the documented 8 rows + columns", {
  expect_s3_class(polis_tables_mapping, "data.frame")
  expect_equal(nrow(polis_tables_mapping), 8L)
  expect_setequal(
    names(polis_tables_mapping),
    c("table_name", "endpoint", "date_field")
  )
  expect_setequal(
    polis_tables_mapping$table_name,
    c(
      "virus",
      "case",
      "human_specimen",
...
test_that("get_polis_data skips a fully-up-to-date table cleanly", {
...
test_that("get_polis_data return = \"paths\" returns the canonical file", {
  root <- withr::local_tempdir()
  seed_parts(
    polis_folder = root,
    table_name = "im",
  ...
  expect_named(paths, "im")
  expect_match(paths[["im"]], "im\\.rds$")
  expect_true(file.exists(paths[["im"]]))
})

test_that("get_polis_data return = \"invisible\" yields NULL", {
  root <- withr::local_tempdir()
  seed_parts(
    polis_folder = root,
    table_name = "im",
...
test_that("force = TRUE deletes existing parts before running", {
  root <- withr::local_tempdir()
  ...
  expect_false(file.exists(part_file))
})
...
test_that("get_polis_data smoke test: pulls `im` end-to-end", {
  skip_on_cran()
  skip_on_ci()
  skip_if_offline()
  key <- Sys.getenv("POLIS_API_KEY")
  skip_if(!nzchar(key), "POLIS_API_KEY not set")
  ...
  df <- tryCatch(
    ...
    error = function(e) {
      ...
      if (transient) skip(paste("POLIS unreachable:", msg))
      stop(e)
    }
  )
